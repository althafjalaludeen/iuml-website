import { PathanamthittaTimelineEntry } from "@/src/data/history/pathanamthitta/types";
import { getPathanamthittaSourcesByIds } from "@/src/data/history/pathanamthitta/sources";
import { SectionHeading } from "@/src/components/ui/SectionHeading";
import { cn } from "@/src/lib/cn";

type PathanamthittaTimelineProps = {
  entries: PathanamthittaTimelineEntry[];
};
...
const typeLabels: Record<string, string> = {
  ...
  election: "Electoral Participation",
  "affiliated-wing": "Affiliated Wings",
  leadership: "Leadership Council",
};

export function PathanamthittaTimeline({ entries }: PathanamthittaTimelineProps) {
  return (
    <section aria-labelledby="pathanamthitta-timeline-heading" className="space-y-10">
      <div className="max-w-3xl">
        <SectionHeading
          id="pathanamthitta-timeline-heading"
          eyebrow="Chronological Progression"
          title="Organizational Timeline"
          description="A documented chronological record of the party's institutional emergence, territorial restructuring in 1982, alliance coordination, and democratic development in Pathanamthitta."
        />
      </div>
      ...
      <div className="relative pt-4 max-w-4xl mx-auto lg:mx-0">
        {entries.map((entry, index) => {
          ...
          const sources = getPathanamthittaSourcesByIds(entry.sourceIds);
                  ...
                  <div className="mt-4 space-y-3 text-sm text-charcoal/80 leading-relaxed pl-3 border-l-2 border-primary-green/30">
                    ...
                    {entry.people && entry.people.length > 0 && (
                      <div className="pt-2 text-xs text-muted">
                        <strong className="text-charcoal font-medium">Documented Figures: </strong>
                        {entry.people.join(", ")}
                      </div>
                    )}
                  </div>
